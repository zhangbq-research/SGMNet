...
_MINI_IMAGENET_DATASET_DIR = './datasets'
...
class UCML(data.Dataset):
    def __init__(self, phase='train', do_not_use_random_transf=False):

        self.base_folder = 'ucml'
        assert(phase=='train' or phase=='val' or phase=='test')
        self.phase = phase
        self.name = 'UCML_' + phase

        print('Loading UCML dataset - phase {0}'.format(phase))
        ...
        if self.phase=='train':
            ...
            self.catname2label = data_train['catname2label']
            ...
            self.catname2label = data_novel['catname2label']
            self.label2catname = {v:k for k, v in self.catname2label.items()}
